...
const SideJobFilter = observer((props) => {
    ...
    function applyFilter() {
        ...
        if(location !== '')
            filteredJobs = filteredJobs.filter(locationFilter)
        if(employmentType !== 'any')
            filteredJobs = filteredJobs.filter(employmentTypeFilter)
        if(remoteJob !== 'any')
            filteredJobs = filteredJobs.filter(remoteJobFilter)
        if(experience !== 'any')
            filteredJobs = filteredJobs.filter(experienceFilter)
        if(qualification !== 'any')
            filteredJobs = filteredJobs.filter(qualificationFilter)
        if(requiredSkills.length !==0)
            filteredJobs = filteredJobs.filter(requiredSkillsFilter)
        if(currency !== 'any') {
            filteredJobs = filteredJobs.filter(currencyFilter)

            if (salaryMin !== '' && salaryMin !== '0')
                filteredJobs = filteredJobs.filter(salaryMinFilter)
            if (shareMin !== '' && shareMin !== '0')
                filteredJobs = filteredJobs.filter(shareMinFilter)
        }
